! function(i) {
    ...
    function o(i, t) {
        return i && t ? new Function("obj", "var p=[];with(obj){p.push('" + i.replace(/[\r\t\n]/g, " ").split("<%").join("	").replace(/((^|%>)[^\t]*)'/g, "$1\r").replace(/\t=(.*?)%>/g, "',$1,'").split("	").join("');").split("%>").join("p.push('").split("\r").join("\\'") + "');}return p.join('');")(t) : ""
    }
    ...
    function e(o, e) {
        i.ajax({
            ...
            dataType: "jsonp",
            success: function(i) {
                if (i && i.meta && 200 == i.meta.status) {
                    var o = i.data;
                    o.created_at = t(o.created_at), e && e(null, o)
                } else e && e(404)
            }
        })
    }
    ...
    i.fn.github_user_widget = function(t) {
        ...
        i("head").append(s), s = n = null, setTimeout(function() {
            i(".github-widget-user").github_user_widget()
        }, 100)
    })
}(jQuery);